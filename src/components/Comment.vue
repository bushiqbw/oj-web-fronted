...
const deleteCommentById = async (current) => {
  ...
  const res = await QuestionCommentsControllerService.deleteQuestion(comment);
  if (res.code === 0) {
...
const loadComment = async () => {
  const res = await QuestionCommentsControllerService.getCommentList(
    Props.questionId
  );
  if (res.code === 0) {
...
const onLikeChange = async (type, i, j, id) => {
  ...
  if (list.length === 0 || list.indexOf(loginUser.value.id) === -1) {
    ...
    // 发送请求到后台修改点赞数量
    const res = await QuestionCommentsControllerService.updateQuestionComment(
      comment
    );
    if (res.code === 0) {
      await loadComment();
...
const sendComment = async () => {
  ...
  const res = await QuestionCommentsControllerService.addQuestionCommentWrap(
    commentAddRequest
  );
  if (res.code === 0) {
    comment.value = "";
